export const getVisibleMedias = state => {
    const vectorMedias = state.sources['medias-source'].metadata.renderedFeatures.map(feature => {
        const featureClone = Object.assign(Object.create(Object.getPrototypeOf(feature)), feature);
    ...
    if (state.sources['selected-medias-source'].metadata.selectFilterPending) {
        const stillFiltered = state.sources['selected-medias-source'].metadata.stillFiltered.map(
            feature => {
                const featureClone = Object.assign(
                    Object.create(Object.getPrototypeOf(feature)),
                    feature
                );
                featureClone.properties.selected = false;
                return featureClone;
        ...
        const selectedFeatures = state.sources['selected-medias-source'].data.features.map(
            feature => {
...
export const getMediasMinDate = state => {
    if (
        ...
        return state.sources['grid-medias-source'].metadata.renderedFeatures.reduce(
            (min, feature) => {
