...
export const selectMediaById = ({ mediaId, ctrlKey, isAdmin }) => {
    return {
        ...
        meta: {
            validator: {
                mediaId: {
                    func: _mediaId => typeof _mediaId === 'string',
...
export const updateTimelineMedias = ({ value }) => {
    return {
        ...
        meta: {
            validator: {
                value: {
                    func: _value => typeof _value === 'number',
...
export const deleteMedias = medias => {
    return dispatch => {
        ...
        const errorMedias = [];
        const data = [];
        const promise = medias.reduce((_promise, media, index) => {
            return _promise
                .then(() => {
                    dispatch({
                ...
                .catch(error => {
                    ...
                    let errorMessage = 'Error deleting media [' + media.properties.name + ']';
        ...
        return promise.then(() => {
            const error =
                errorMedias.length > 0 ? { medias: errorMedias, messages: errorMessages } : null;
            dispatch({
...
export const uploadMedias = (files, position) => {
    return dispatch => {
        ...
        const errorFiles = [];
        const data = [];
        const promise = iter.reduce((_promise, item, index) => {
            return _promise
                .then(() => {
                    dispatch({
                ...
                .catch(error => {
                    const response = error.response;
                    let errorMessage = 'Error uploading file [' + files[index].name + ']';
                    if (response && response.data && response.data.message) {
                        errorMessage += ' : ' + error.response.data.message;
                    }
        ...
        return promise.then(() => {
            const error =
                errorFiles.length > 0 ? { files: errorFiles, messages: errorMessages } : null;
            dispatch({
...
function _uploadMedia(file, currentPosition) {
    return new Promise((resolve, reject) => {
        ...
        reader.onloadend = e => {
            const exifArrayBuffer = e.target.result,
        ...
        reader.readAsArrayBuffer(file.slice(0, 90000, file.type));
    });
}
...
function getMediaPosition(arrayBuffer) {
    try {
        if (arrayBuffer) {
            const parser = EXIF.create(arrayBuffer);
